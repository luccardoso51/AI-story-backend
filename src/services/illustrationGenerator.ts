...
function createIllustrationPrompt(prompt: IllustrationPrompt): string {
  ...
  if (type === 'cover') {
    return `Create a colorful, child-friendly illustration for a children's story titled "${storyTitle}". 
    The image should be engaging, bright, and suitable for children. 
    IMPORTANT INSTRUCTIONS:
    1. Create ONE single cohesive illustration for the cover of the book.
    ...
    8. Include key elements or characters from the story.
    ...
    6. Include key elements or characters from the story.
